...
export default function Home() {
  const [step, setStep] = useState(1); // 1: القائمة, 2: النشر, 3: معاينة الإعلان
  ...
  // حالات خاصة بمعاينة الإعلان الداخلي
  ...
  // التحكم في العداد التنازلي
  useEffect(() => {
    let interval;
    if (step === 3 && timer > 0) {
      interval = setInterval(() => setTimer(t => t - 1), 1000);
    } else if (timer === 0 && activeAd) {
      completeVisit(activeAd);
    }
  ...
  const startAdPreview = (ad) => {
    setActiveAd(ad);
    setTimer(30);
    setStep(3); // الانتقال لواجهة المعاينة
  };

  const completeVisit = async (ad) => {
    // تحديث قاعدة البيانات
    if (ad.id) {
      await supabase.from('ads').update({ clicks: (ad.clicks || 0) + 1 }).eq('id', ad.id);
    }
    // إضافة الزيارة للقائمة المطلوبة للنشر
    setClickedLinks(prev => new Set(prev).add(ad.id));
    alert("✅ تمت مشاهدة الإعلان بنجاح، يمكنك العودة الآن.");
  };

  const handlePublish = async (e) => {
    e.preventDefault();
    setLoading(true);
    const { error } = await supabase.from('ads').insert([{ ...formData, clicks: 0 }]);
    if (!error) {
      ...
      <div className="fixed inset-0 bg-white z-[100] flex flex-col font-sans" dir="rtl">
        <div className="p-4 bg-slate-900 text-white flex justify-between items-center shadow-xl">
          <div className="flex flex-col">
            <span className="text-xs font-black text-blue-400 uppercase tracking-widest">إعلان مميز</span>
            <span className="font-bold truncate max-w-[150px]">{activeAd?.title}</span>
          </div>
          <div className="flex items-center gap-4">
            <div className={`px-4 py-2 rounded-full font-black text-sm border-2 ${timer > 0 ? 'border-yellow-400 text-yellow-400' : 'border-green-400 text-green-400 animate-bounce'}`}>
              {timer > 0 ? `انتظر ${timer} ثا` : "جاهز ✓"}
            </div>
            <button onClick={() => setStep(1)} className="bg-red-500 hover:bg-red-600 px-4 py-2 rounded-xl font-black text-sm transition">إغلاق</button>
          </div>
        </div>
        <div className="h-1 bg-slate-800">
           <div className="h-full bg-blue-600 transition-all duration-1000" style={{ width: `${(30 - timer) / 30 * 100}%` }}></div>
        </div>
        <iframe src={activeAd?.url} className="flex-grow w-full border-none" title="Ad Content" />
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans flex flex-col" dir="rtl">
      <nav className="p-4 bg-white border-b border-slate-200 shadow-sm flex justify-between items-center sticky top-0 z-50">
        ...
        <button onClick={() => setStep(step === 1 ? 2 : 1)} className="bg-blue-600 text-white px-6 py-2 rounded-xl font-black text-sm hover:bg-blue-700 shadow-md transition-all">
      ...
      <main className="max-w-xl mx-auto p-4 flex-grow w-full">
        {step === 1 ? (
          <div className="space-y-4">
            <h2 className="text-center font-black text-slate-700 py-4">الإعلانات النشطة</h2>
            {allAds.map((ad, i) => (
              <div key={ad.id || i} className="bg-white p-5 rounded-[2rem] shadow-sm border border-slate-100 hover:border-blue-500 transition-all cursor-pointer group" onClick={() => startAdPreview(ad)}>
                ...
                <div className="mt-4 text-[12px] text-slate-400 font-black">بواسطة: {ad.name}</div>
              </div>
            ))}
          </div>
        ) : (
          <div className="bg-white p-8 rounded-[3rem] shadow-2xl border border-slate-100">
             ...
             {allAds.length > 0 && (
               <div className="space-y-3 mb-8">
                <p className="text-xs text-center text-slate-500 mb-4 font-black">شاهد 5 إعلانات (30 ثانية لكل منها): ({clickedLinks.size}/{Math.min(allAds.length, 5)})</p>
                {allAds.slice(0, 5).map((ad, i) => (
                  <button key={i} onClick={() => startAdPreview(ad)} className={`w-full p-4 rounded-2xl border-2 text-right flex justify-between items-center transition-all ${clickedLinks.has(ad.id) ? 'bg-green-50 border-green-500 text-green-700' : 'bg-slate-50 border-slate-50 hover:border-blue-400 text-slate-700'}`}>
                    <span className="font-black text-sm">{ad.title}</span>
                    {clickedLinks.has(ad.id) ? <span className="text-xs font-black">جاهز ✓</span> : <span className="text-xs font-black text-blue-600 font-black underline">مشاهدة الآن</span>}
                  </button>
                ))}
              </div>
             )}
      ...
      <footer className="bg-white border-t border-slate-200 p-8 text-center">
        <p className="text-slate-800 font-black text-base">&copy; 2026 جميع الحقوق محفوظة لموقع <span className="text-blue-600 shadow-blue-200">TRAFFIC-DZ</span></p>
      </footer>
    </div>
  );
}
